...
type TimerState = { tasks: Task[]; categories: TaskTag[]; lastTimestamp: number; breakState?: BreakState; workSessionStartedAt?: number; breakReminderShown?: boolean; dismissedHints: HintId[] };
type TimerApi = TimerState & { ready: boolean; now: number; addTask: (title: string, tagId: TaskTagId) => void; start: (id: string) => void; pause: (id: string) => void; complete: (id: string) => void; restart: (id: string) => void; startBreak: () => void; endBreak: () => void; dismissBreakReminder: () => void; dismissHint: (hint: HintId) => void; copyLatestTasks: () => void; updateTask: (id: string, title: string, tagId: TaskTagId) => void; updateAllocatedDuration: (id: string, allocatedDuration: number) => void; addCategory: (label: string, iconUri?: string, systemIconId?: string) => void; updateCategory: (id: string, label: string, iconUri?: string, systemIconId?: string) => void; deleteCategory: (id: string) => void; remove: (id: string) => void };
const KEY = 'timeflow-v1'; const TimerContext = createContext<TimerApi | null>(null);
const initial: TimerState = { tasks: [], categories: createStarterTaskTags(), lastTimestamp: Date.now(), dismissedHints: [] };
...
function settleTimerState(source: TimerState, at: number): TimerState {
  if (source.breakState) return { ...source, lastTimestamp: at };
